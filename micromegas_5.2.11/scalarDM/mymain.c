...
// #endif
...
#ifdef OMEGA
	{ int fast=1;
	double Beps=1e-6, cut=0.01;
	double Omega;
	double Omega_onechan;  
	int i, err; 
	//printf("\n==== Calculation of relic density =====\n");
	double Xf;
	double Y;
  ...
  file = fopen(filename,"w");
	...
	Omega=darkOmega(&Xf,fast,Beps,&err);
	//Omega=darkOmegaFO(&Xf,fast,Beps);
	//Omega_onechan = oneChannel(Xf,Beps,"~DM","~dm","e-","e+");
	// for (i = 0; i < 5; ++i){
	// Y = YF(Tl[i]);
	// printf("%.2e",Y);};

	FILE *channels;
	char filename_chan[100];
	snprintf(filename_chan, sizeof(filename_chan), "channels/channels_mdm%.2e_eps%.3e.dat",findValW("mdm"),findValW("eps"));
	printf("m_chi = %.2e, mAp = %.2e", findValW("mdm"),findValW("mAp"));
...
#ifdef CROSS_SECTIONS
{
  ...
  { 
	...
	int i;
	float T;
	float Ti = -3.0; //10^Ti: min temperature
	...
	// FILE *geff;
	// char filename_geff[25];
	// snprintf(filename_geff, sizeof(filename_geff), "gEff.dat");
	// geff = fopen(filename_geff,"w");
